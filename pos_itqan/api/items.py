...
def get_item_detail(item, doc=None, warehouse=None, price_list=None, company=None):
	...
	# Look up template using standard ERPNext logic
	tax_template = None
...
def get_items(pos_profile, search_term=None, item_group=None, start=0, limit=20):
	...
	try:
		...
		if search_term and len(search_term.strip().split()) == 1:
			from pos_itqan.services.barcode import resolve_barcode
		...
		if item_codes:
			try:
				...
				for item_code in set(item_codes):
					...
					for tax in item_doc.get("taxes") or []:
						tax_company = frappe.get_cached_value("Item Tax Template", tax.item_tax_template, "company")
						if not tax_company or tax_company == pos_company:
							tax_template = tax.item_tax_template
							break
					...
					if tax_template:
						item_to_template[item_code] = tax_template
						template_names.add(tax_template)
		...
		if resolved_barcode_data and items:
			from pos_itqan.services.barcode import compute_resolved_item_data
