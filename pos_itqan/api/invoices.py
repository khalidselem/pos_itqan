...
def _populate_item_tax_data(invoice_doc, pos_profile):
    ...
    try:
        ...
        # ── Step 3: Ensure ALL required tax accounts are in doc.taxes ──
        ...
        # Check if POS Profile expects taxes to be inclusive (safely)
        tax_inclusive_price = 0
        if pos_profile:
            try:
                if frappe.db.has_column("POS Profile", "tax_inclusive_price"):
                    tax_inclusive_price = frappe.db.get_value("POS Profile", pos_profile, "tax_inclusive_price") or 0
                elif frappe.db.has_column("POS Profile", "tax_inclusive_tax"):
                    tax_inclusive_price = frappe.db.get_value("POS Profile", pos_profile, "tax_inclusive_tax") or 0
            except Exception:
                pass
    ...
    try:
        ...
        if coupon_code:
            ...
            if frappe.db.table_exists("POS Coupon"):
                from pos_itqan.pos_itqan.doctype.pos_coupon.pos_coupon import check_coupon_code
...
def check_offline_invoice_synced(offline_id):
    ...
    """
    from pos_itqan.pos_itqan.doctype.offline_invoice_sync.offline_invoice_sync import (
        OfflineInvoiceSync,
    )
...
def submit_invoice(invoice=None, data=None):
    ...
    try:
        ...
        if coupon_code:
            ...
            if frappe.db.table_exists("POS Coupon"):
                try:
                    from pos_itqan.pos_itqan.doctype.pos_coupon.pos_coupon import increment_coupon_usage
        ...
        if redeemed_customer_credit and customer_credit_dict:
            try:
                from pos_itqan.api.credit_sales import redeem_customer_credit
