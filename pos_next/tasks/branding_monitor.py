...
def monitor_branding_integrity():
	...
	"""
	try:
		# Check if ITQAN Branding doctype exists
		if not frappe.db.exists("DocType", "ITQAN Branding"):
			return
		...
		if not doc.encrypted_signature:
			frappe.log_error(
				title="ITQAN Branding - Missing Signature",
				message="Branding configuration is missing encrypted signature. Please resave the ITQAN Branding document."
			)

		# Log monitoring activity
		frappe.logger().info(f"ITQAN Branding Monitor - Checked at {now()}, Tampering attempts: {doc.tampering_attempts or 0}")

	except Exception as e:
		frappe.log_error(
			title="ITQAN Branding Monitor Error",
...
def send_tampering_alert(doc):
	...
	try:
		...
		# Create notification
		message = f"""
<h3>ITQAN Branding - Excessive Tampering Detected</h3>
<p>The ITQAN branding footer has been tampered with <strong>{doc.tampering_attempts}</strong> times.</p>
...
<p><a href="/app/itqan-branding">View Branding Configuration</a></p>
		"""
		...
		for user in users:
			try:
				frappe.get_doc({
					...
					"subject": "ITQAN Branding - Tampering Alert",
					"type": "Alert",
					"document_type": "ITQAN Branding",
		...
		frappe.log_error(
			title="ITQAN Branding Alert Error",
...
def reset_tampering_counter():
	...
	try:
		...
		if old_count > 0:
			frappe.log_error(
				title="ITQAN Branding - Monthly Counter Reset",
				message=json.dumps({
		...
		# Reset counter
		frappe.db.set_value("ITQAN Branding", doc.name, "tampering_attempts", 0)
		frappe.db.commit()

	except Exception as e:
		frappe.log_error(
			title="ITQAN Branding Counter Reset Error",
...
def validate_all_active_sessions():
	...
	try:
		...
		if active_sessions:
			frappe.logger().info(
				f"ITQAN Branding - Active sessions: {len(active_sessions)}"
			)

	except Exception as e:
		frappe.log_error(
			title="ITQAN Branding Session Validation Error",
