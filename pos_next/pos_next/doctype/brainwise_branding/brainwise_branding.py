...
class ITQANBranding(Document):
	...
	def validate(self):
		...
		if not self.is_new():
			...
			if protected_fields_changed:
				...
				frappe.log_error(
					title="ITQAN Branding - Fields Modified with Master Key",
					message=json.dumps({
		...
		if not self.enabled and not self.is_new():
			if not self.master_key_provided or not self._validate_master_key():
				frappe.throw(
					...
					"Contact ITQAN LLC support (info@itqan-kw.net) if you need to disable branding.",
					frappe.PermissionError
				)
	...
	def _validate_master_key(self):
		...
		try:
			...
			if key_hash == MASTER_KEY_HASH and phrase_hash == PROTECTION_PHRASE_HASH:
				...
				frappe.log_error(
					title="ITQAN Branding - Master Key Used",
					message=json.dumps({
						"user": frappe.session.user,
						"timestamp": frappe.utils.now(),
						"ip_address": frappe.local.request_ip if hasattr(frappe.local, 'request_ip') else None,
			...
			# Log failed attempt
			frappe.log_error(
				title="ITQAN Branding - Invalid Master Key Attempt",
				message=json.dumps({
					"user": frappe.session.user,
					"timestamp": frappe.utils.now(),
					"ip_address": frappe.local.request_ip if hasattr(frappe.local, 'request_ip') else None,
			...
			frappe.log_error(f"Master key validation error: {str(e)}", "ITQAN Branding")
			return False
	...
	def validate_signature(self, client_data):
		...
		try:
			...
			frappe.log_error(f"Branding validation error: {str(e)}", "ITQAN Branding")
			return False
	...
	def log_tampering(self, details):
		...
		# Create error log
		frappe.log_error(
			title="ITQAN Branding Tampering Detected",
...
def get_branding_config():
	...
	try:
		doc = frappe.get_single("ITQAN Branding")
		...
		frappe.log_error(f"Error fetching branding config: {str(e)}", "ITQAN Branding")
		# Return default config even on error
		return {
			"_t": base64.b64encode("Powered by".encode()).decode(),
			"_l": base64.b64encode("ITQAN LLC".encode()).decode(),
			"_u": base64.b64encode("https://itqan-kw.com".encode()).decode(),
			"_i": 10000,
			"_v": True,
			"_e": 1
		}
...
def validate_branding(client_signature=None, brand_name=None, brand_url=None):
	...
	try:
		...
		frappe.log_error(f"Error validating branding: {str(e)}", "ITQAN Branding")
...
def log_client_event(event_type=None, details=None):
	...
	try:
		...
		frappe.log_error(f"Error logging client event: {str(e)}", "ITQAN Branding")
...
def verify_master_key(master_key_input):
	...
	try:
		...
		frappe.log_error(
			title=f"ITQAN Branding - Master Key Verification {'Success' if is_valid else 'Failed'}",
			message=json.dumps({
				"user": frappe.session.user,
				"timestamp": frappe.utils.now(),
		...
		frappe.log_error(f"Master key verification error: {str(e)}", "ITQAN Branding")
		return {
			"valid": False,
			"error": str(e)
		}
...
def generate_new_master_key():
	...
	# Log this generation
	frappe.log_error(
		title="ITQAN Branding - New Master Key Generated",
		message=json.dumps({
			"user": frappe.session.user,
			"timestamp": frappe.utils.now(),
	...
	return {
		...
		"instructions": "IMPORTANT: Save these securely! Update MASTER_KEY_HASH and PROTECTION_PHRASE_HASH in itqan_branding.py with the hashes provided above."
	}
